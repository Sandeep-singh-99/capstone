from fastapi import APIRouter, Depends, HTTPException, Response, status, Request, File, Form, UploadFile
...
async def login_user(request: Request, response: Response, db: Session = Depends(get_db), user_sign_in: userSignIn = Depends()):
    ...
    _, access_token = AuthService.authenticate_user(
        ...
        hashed_password=user_sign_in.hashed_password
    )
...
async def logout_user(request: Request, response: Response, current_user: User = Depends(get_current_user)):
